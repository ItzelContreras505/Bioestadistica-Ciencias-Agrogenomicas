...
# 1. Simular muestra de 500 estudiantes
set.seed(123)  # Para reproducibilidad
alturas <- rnorm(n, mean = mu, sd = sigma)

# 2. Calcular estadísticos muestrales
media_muestral <- mean(alturas)
mediana_muestral <- median(alturas)
desv_muestral <- sd(alturas)

cat("Media muestral:", media_muestral, "\n")
cat("Mediana muestral:", mediana_muestral, "\n")
cat("Desviación estándar muestral:", desv_muestral, "\n")

# 3. Porcentaje de estudiantes que miden más de 180 cm
porcentaje_mayor_180 <- mean(alturas > 180) * 100
cat("Porcentaje que mide más de 180 cm:", porcentaje_mayor_180, "%\n")

# 4. Altura del percentil 25
percentil_25 <- quantile(alturas, 0.25)
cat("Altura en el percentil 25:", percentil_25, "cm\n")

# 5. Histograma con densidad teórica superpuesta
...
ggplot(df, aes(x = altura)) +
  ...
  labs(title = "Histograma de alturas con densidad teórica",
       x = "Altura (cm)",
       y = "Densidad") +
  theme_minimal()
...
```{r}
# Ejercicio 5.1 - Control de Temperatura Industrial
...
mu <- 350
sigma <- 5
temperaturas <- rnorm(n, mean = mu, sd = sigma)

# 2. ¿Qué porcentaje del tiempo la temperatura está fuera de especificación?
lim_inf <- 340
lim_sup <- 360
fuera_especificacion <- sum(temperaturas < lim_inf | temperaturas > lim_sup)
porcentaje_fuera <- (fuera_especificacion / n) * 100
cat("Porcentaje fuera de especificación:", porcentaje_fuera, "%\n")

# 3. Si el proceso se descentra a μ = 355°C, ¿cómo cambia la calidad?
temperaturas_descentradas <- rnorm(n, mean = 355, sd = sigma)
fuera_descentrado <- sum(temperaturas_descentradas < lim_inf | temperaturas_descentradas > lim_sup)
porcentaje_fuera_descentrado <- (fuera_descentrado / n) * 100
cat("Porcentaje fuera de especificación con μ = 355°C:", porcentaje_fuera_descentrado, "%\n")

# 4. ¿Qué se necesita para tener 99.9% de productos dentro de especificación?
# Queremos que P(340 <= X <= 360) = 0.999
# Usamos la distribución normal estándar para encontrar el z correspondiente
z <- qnorm(0.9995)  # 99.9% central → 0.9995 en cada extremo
# Entonces: (360 - 350) = z * sigma → sigma = (360 - 350) / z
sigma_requerido <- (360 - 350) / z
cat("Desviación estándar requerida para 99.9% dentro de especificación:", round(sigma_requerido, 2), "°C\n")
...
# 1. Simula las ventas de 24 meses
ventas <- rnorm(meses, mean = mu_ventas, sd = sd_ventas)

# 2. Probabilidad de ventas > 60,000 pesos
prob_mayor_60000 <- 1 - pnorm(60000, mean = mu_ventas, sd = sd_ventas)
cat("Probabilidad de ventas > 60,000:", round(prob_mayor_60000 * 100, 2), "%\n")

# 3. ¿Cuántos meses se espera tener ventas < 30,000?
prob_menor_30000 <- pnorm(30000, mean = mu_ventas, sd = sd_ventas)
meses_menores_30000 <- round(prob_menor_30000 * meses)
cat("Meses esperados con ventas < 30,000:", meses_menores_30000, "\n")

# 4. Rango intercuartílico (IQR)
iqr <- IQR(ventas)
cat("Rango intercuartílico:", round(iqr, 2), "pesos\n")

# 5. Porcentaje de meses con ventas > 45,000
porcentaje_mayor_45000 <- mean(ventas > 45000) * 100
cat("Porcentaje de meses con ventas > 45,000:", round(porcentaje_mayor_45000, 2), "%\n")
...
# 1. Simula la vida útil de 800 baterías
vida_util <- rnorm(n_baterias, mean = mu_bateria, sd = sd_bateria)

# 2. ¿Qué garantía (en horas) cubre al 95% de las baterías?
garantia_95 <- qnorm(0.05, mean = mu_bateria, sd = sd_bateria)
cat("Garantía que cubre al 95% de las baterías:", round(garantia_95, 2), "horas\n")

# 3. Si se venden 10,000 baterías con garantía de 900 horas, ¿cuántas se espera que fallen?
prob_falla <- pnorm(900, mean = mu_bateria, sd = sd_bateria)
fallas_esperadas <- round(prob_falla * 10000)
cat("Baterías que se espera fallen con garantía de 900 horas:", fallas_esperadas, "\n")

# 4. Compara la media y mediana de la simulación con los valores teóricos
media_sim <- mean(vida_util)
mediana_sim <- median(vida_util)
cat("Media simulada:", round(media_sim, 2), "horas\n")
cat("Mediana simulada:", round(mediana_sim, 2), "horas\n")
cat("Media teórica:", mu_bateria, "horas\n")
cat("Mediana teórica:", mu_bateria, "horas\n")  # En normal, media = mediana

# 5. ¿Cuál es el percentil 10 de la distribución?
percentil_10 <- qnorm(0.10, mean = mu_bateria, sd = sd_bateria)
...
# 1. Simula 1000 mediciones
errores <- rnorm(n_mediciones, mean = mu_error, sd = sd_error)

# 2. ¿Qué porcentaje de errores está entre -5 y +5 unidades?
dentro_rango <- sum(errores >= -5 & errores <= 5)
porcentaje_dentro <- (dentro_rango / n_mediciones) * 100
cat("Porcentaje de errores entre -5 y +5:", round(porcentaje_dentro, 2), "%\n")

# 3. Probabilidad de error > 6 unidades en valor absoluto
# Es decir: P(|X| > 6) = P(X < -6) + P(X > 6)
prob_error_abs_mayor_6 <- pnorm(-6, mean = mu_error, sd = sd_error) + 
                          (1 - pnorm(6, mean = mu_error, sd = sd_error))
cat("Probabilidad de error > 6 unidades (valor absoluto):", round(prob_error_abs_mayor_6 * 100, 4), "%\n")

# 4. Porcentaje de mediciones "precisas" (error < 3 unidades en valor absoluto)
precisas <- sum(abs(errores) < 3)
porcentaje_precisas <- (precisas / n_mediciones) * 100
cat("Porcentaje de mediciones precisas:", round(porcentaje_precisas, 2), "%\n")

# 5. Verifica que la distribución es simétrica alrededor de 0
# Comparamos la cantidad de errores negativos vs positivos
negativos <- sum(errores < 0)
positivos <- sum(errores > 0)
cat("Errores negativos:", negativos, "\n")
cat("Errores positivos:", positivos, "\n")

# También podemos graficar para visualizar la simetría
hist(errores, breaks = 30, col = "lightgray", main = "Distribución de errores", xlab = "Error (unidades)")
...
# Grupo A: N(75, 10^2)
grupo_A <- rnorm(n_estudiantes, mean = 75, sd = 10)

# Grupo B: N(82, 8^2)
grupo_B <- rnorm(n_estudiantes, mean = 82, sd = 8)

# 2. ¿Qué porcentaje del Grupo A supera la media del Grupo B?
media_B <- 82
superan_B <- sum(grupo_A > media_B)
porcentaje_superan <- (superan_B / n_estudiantes) * 100
cat("Porcentaje del Grupo A que supera la media del Grupo B:", round(porcentaje_superan, 2), "%\n")

# 3. Probabilidad de que un estudiante del Grupo B tenga puntuación < 70
prob_B_menor_70 <- pnorm(70, mean = 82, sd = 8)
cat("Probabilidad de que un estudiante del Grupo B tenga < 70:", round(prob_B_menor_70 * 100, 2), "%\n")

# 4. Comparar las distribuciones visualmente
hist(grupo_A, col = rgb(1, 0, 0, 0.5), xlim = c(40, 110), breaks = 20,
     main = "Distribuciones de Puntuaciones", xlab = "Puntaje")
hist(grupo_B, col = rgb(0, 0, 1, 0.5), breaks = 20, add = TRUE)
legend("topright", legend = c("Grupo A", "Grupo B"), fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))

# 5. ¿A partir de qué puntuación está el 10% superior de cada grupo?
percentil_90_A <- qnorm(0.90, mean = 75, sd = 10)
percentil_90_B <- qnorm(0.90, mean = 82, sd = 8)
cat("Top 10% Grupo A: desde", round(percentil_90_A, 2), "puntos\n")
cat("Top 10% Grupo B: desde", round(percentil_90_B, 2), "puntos\n")
