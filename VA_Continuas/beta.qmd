...
for (i in seq_along(tamaños)) {
  muestra <- rbeta(tamaños[i], alpha, beta)
  medias_muestrales[i] <- mean(muestra)
  varianzas_muestrales[i] <- var(muestra)
}

# Gráfica de convergencia de la media
g1 <- ggplot(data.frame(tamaño = tamaños, media = medias_muestrales), aes(x = tamaño, y = media)) +
...
g2 <- ggplot(data.frame(tamaño = tamaños, varianza = varianzas_muestrales), aes(x = tamaño, y = varianza)) +
  geom_line(color = "darkgreen") +
  geom_hline(yintercept = var_teorica, linetype = "dashed", color = "red") +
...
# Histograma con PDF teórica para la muestra más grande
muestra_grande <- rbeta(max(tamaños), alpha, beta)
df_hist <- data.frame(x = muestra_grande)

ggplot(df_hist, aes(x)) +
  ...
  labs(title = "Histograma de la muestra (n = 10000) con PDF Beta(2,5)",
       x = "Valor",
       y = "Densidad") +
  theme_minimal()
...
# Simulación para cada tamaño
for (n in tamaños) {
  medias <- replicate(n_rep, mean(rbeta(n, alpha, beta)))
  resultados[[as.character(n)]] <- medias
}

# Graficar histogramas de las medias muestrales
graficas <- list()
for (i in seq_along(tamaños)) {
  n <- tamaños[i]
  df <- data.frame(media = resultados[[as.character(n)]])
  g <- ggplot(df, aes(x = media)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black") +
    stat_function(fun = dnorm,
                  args = list(mean = media_teorica,
                              sd = sqrt(var_teorica / n)),
                  color = "red", size = 1) +
...
do.call(grid.arrange, c(graficas, ncol = 2))
