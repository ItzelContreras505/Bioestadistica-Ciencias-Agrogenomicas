...
for (k in 1:muestras){
df_previa <- data.frame(
Muestra = k,
Valor =sum(rexp(n,rate =1.5))
)
df_exr <- bind_rows(df_exr,df_previa)
}

ggplot(df_exr)+
geom_histogram(aes(x= Valor, y = after_stat(density)),fill ="#EAD235", color="black", alpha=0.5, boundary = 0)+
geom_function(fun = dgamma, args = list(shape =n, rate = 1.5), linewidth =1.2, color ="red", xlim=c(0,10.5))+
theme_bw()
```
...
df_expr <- bind_rows(df_exr,df_previa)
}

ggplot(df_exr),
geom_histogram(aes(x= valor, y = after_stat(density)),fill ="YEAD235", color="black", alpha=0.5, bondary = 0)= theme_bw()
