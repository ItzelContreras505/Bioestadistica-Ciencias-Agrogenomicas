...
# Generar muestra de 1000 tiempos de carga
set.seed(123)
tiempos_carga <- runif(1000, min = a, max = b)

# Crear histograma con densidad teórica
ggplot(data.frame(Tiempo = tiempos_carga), aes(x = Tiempo)) +
  ...
  labs(title = "Distribución de Tiempos de Carga de Página Web",
       subtitle = "1000 observaciones de distribución Uniforme(2,8)",
...
# 1. Simular calificaciones de 500 estudiantes
set.seed(123)
calificaciones <- runif(500, min = min_calif, max = max_calif)

# 2. Porcentaje con calificación superior a 85
porcentaje_superior_85 <- mean(calificaciones > 85) * 100
porcentaje_teorico_85 <- (100 - 85) / (100 - 70) * 100
...
porcentajes_muestrales <- prop.table(tabla_categorias) * 100

# Porcentajes teóricos (distribución uniforme)
porcentajes_teoricos <- c(10/30 * 100, 10/30 * 100, 10/30 * 100)  # 33.33% cada uno

# 4. Crear dataframe para comparación
comparacion <- data.frame(
  Categoria = c("Aprobado (70-79)", "Bien (80-89)", "Excelente (90-100)"),
  Muestral = as.numeric(porcentajes_muestrales),
  Teorico = porcentajes_teoricos
)

# Mostrar resultados
cat("=== DISTRIBUCIÓN DE CALIFICACIONES ===\n\n")
cat("1. PORCENTAJE CON CALIFICACIÓN > 85:\n")
cat("   Muestral:", round(porcentaje_superior_85, 1), "%\n")
cat("   Teórico: ", round(porcentaje_teorico_85, 1), "%\n\n")

cat("2. DISTRIBUCIÓN POR CATEGORÍAS:\n")
print(comparacion, row.names = FALSE)
cat("\n")

cat("3. RESUMEN ESTADÍSTICO:\n")
cat("   Media:", round(mean(calificaciones), 1), "\n")
cat("   Mínimo:", round(min(calificaciones), 1), "\n")
cat("   Máximo:", round(max(calificaciones), 1), "\n")
...
# Gráfico de densidad con categorías
ggplot(df_calificaciones, aes(x = Calificacion, fill = Categoria)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, 
                 color = "black", alpha = 0.8, position = "identity") +
  geom_density(alpha = 0.3, color = "darkred", linewidth = 1) +
  stat_function(fun = dunif, args = list(min = min_calif, max = max_calif), 
                color = "blue", linewidth = 1, linetype = "dashed") +
  labs(title = "Distribución de Calificaciones - Examen Muy Fácil",
       subtitle = "500 estudiantes - Distribución Uniforme(70,100)",
       x = "Calificación",
       y = "Densidad",
       fill = "Categorías") +
...
# Gráfico de comparación porcentual
ggplot(comparacion, aes(x = Categoria)) +
  geom_bar(aes(y = Muestral, fill = "Muestral"), 
           stat = "identity", position = "dodge", alpha = 0.7) +
  geom_point(aes(y = Teorico, color = "Teórico"), size = 3) +
  geom_line(aes(y = Teorico, group = 1, color = "Teórico"), linewidth = 1) +
  labs(title = "Comparación: Porcentajes Muestrales vs Teóricos",
       y = "Porcentaje (%)",
       x = "Categorías") +
  scale_fill_manual(name = "", values = c("Muestral" = "steelblue")) +
  scale_color_manual(name = "", values = c("Teórico" = "red")) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
